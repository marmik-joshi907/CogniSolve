      ...
      <nav className="hidden md:flex flex-col h-screen w-64 fixed left-0 top-0 z-40 bg-slate-50 dark:bg-slate-900 py-6 px-4 gap-2">
        ...
        <div className="flex-1 flex flex-col gap-1">
          ...
          <a
            className="flex items-center gap-3 px-4 py-3 text-slate-600 dark:text-slate-400 hover:text-indigo-900 dark:hover:text-indigo-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md font-medium font-['Inter'] text-sm tracking-wide uppercase transition-colors"
            href="#"
            onClick={(e) => handleNav(e, "Complaint Log")}
          >
            <span className="material-symbols-outlined text-[20px]">
              history_edu
            </span>
        ...
        <div className="p-6 md:p-10 pt-24 md:pt-10 max-w-7xl mx-auto space-y-10">
          ...
          <header className="mb-8">
            ...
            <p className="font-body text-secondary max-w-2xl">
              Submit complaints, view AI classifications with ML-powered
              resolution recommendations, and manage the active queue.
            </p>
          </header>
          {/* Bento Grid Layout */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 flex flex-col gap-8">
              ...
              <div className="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-outline-variant/15 relative overflow-hidden">
                ...
                <h3 className="font-headline font-bold text-lg mb-4 text-on-surface">
                  New Analysis
                </h3>
                <form className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-md bg-gradient-to-br from-primary to-primary-container text-on-primary font-bold shadow-md hover:shadow-lg transition-all text-sm disabled:opacity-50"
                    type="button"
                  >
                  ...
                  <div className="bg-surface-container-lowest p-5 rounded-lg h-full border border-outline-variant/15">
                    ...
                    <div className="space-y-4">
                      <div className="flex gap-2">
                        <span className="bg-error-container text-on-error-container text-xs font-bold px-2 py-1 rounded-sm uppercase tracking-wider">
                      ...
                      {result.resolution_text && (
                        <div className="bg-[#006d3a]/5 p-3 rounded-md border border-[#006d3a]/15">
                          <h4 className="font-label text-[10px] uppercase tracking-wider text-[#006d3a] mb-2 flex items-center gap-1">
                            ...
                            <span className="text-[9px] bg-[#006d3a]/10 px-1.5 py-0.5 rounded ml-1">
                              {result.resolution_method || "template"}
                            </span>
                          </h4>
                      ...
                      <div className="pt-2 flex justify-end gap-2">
                        <button
                          className="px-4 py-2 text-sm font-medium text-primary hover:bg-surface-container rounded-md transition-colors"
              ...
              <div className="bg-surface-container-lowest rounded-xl shadow-sm border border-outline-variant/15 overflow-hidden">
                <div className="px-6 py-4 border-b border-surface-container flex justify-between items-center">
                  ...
                  <span className="text-xs font-label uppercase text-secondary tracking-wider">
                    Showing {complaints.length} items
                  </span>
                </div>
                    ...
                    complaints.map((comp) => (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                          <div className="w-16 text-xs font-label text-secondary font-medium">
                            #{comp.id}
                          </div>
                          <div className="flex-1">
                            <p className="font-body text-sm font-medium text-on-surface mb-1 truncate max-w-md">
                              {comp.complaint_text}
                            </p>
                            <div className="flex gap-2 items-center flex-wrap">
                              ...
                              <span
                                className={`${comp.priority === "high" ? "bg-error-container text-on-error-container" : comp.priority === "medium" ? "bg-tertiary-fixed text-on-tertiary-fixed" : "bg-[#d1e7dd] text-[#0f5132]"} text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-sm`}
                              >
                                {comp.priority}
                              </span>
                          ...
                          <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
                            ...
                            {comp.status === "open" && (
                              <button
                                onClick={() =>
                                  handleStatusUpdate(comp.id, "in_progress")
                                }
                                className="px-3 py-1.5 text-xs font-bold bg-primary/10 text-primary rounded-md hover:bg-primary/20 transition-colors"
                              >
                                Start
                              </button>
                            )}
                            {comp.status === "in_progress" && (
                              <button
                                onClick={() =>
                                  handleStatusUpdate(comp.id, "resolved")
                                }
                                className="px-3 py-1.5 text-xs font-bold bg-[#006d3a]/10 text-[#006d3a] rounded-md hover:bg-[#006d3a]/20 transition-colors"
                              >
                                Resolve
                              </button>
                            )}
                            <div
                              className={`${comp.status === "resolved" || comp.status === "closed" ? "text-[#006d3a]" : comp.status === "in_progress" ? "text-primary" : "text-error"} text-xs font-bold flex items-center gap-1`}
                            >
                              <span className="material-symbols-outlined text-[14px]">
