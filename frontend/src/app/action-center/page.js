...
const SlaTimer = ({ deadline, initialBreached }) => {
  ...
  if (isBreached) {
    return (
      <div className="flex items-center gap-1.5 text-error font-bold text-[10px] bg-error-container/20 px-1.5 py-0.5 rounded uppercase border border-error/20">
  ...
  return (
    <div className={`flex items-center gap-1 font-bold text-[10px] px-1.5 py-0.5 rounded border ${isWarning ? 'text-orange-700 bg-orange-50 border-orange-200' : 'text-primary bg-primary/5 border-primary/20'}`}>
    ...
    try {
      ...
      if (data.success) {
        setResult({
          ...
          priority_reason: data.classification?.priority_reason || '',
          top_keywords: data.classification?.top_keywords || {},
          class_probabilities: data.classification?.class_probabilities || {},
...
<h4 className="font-label text-[10px] uppercase tracking-wider text-secondary">Target SLA Deadline</h4>
<SlaTimer deadline={result.sla_deadline} initialBreached={result.sla_breached} />
</div>
<p className="font-body text-[10px] text-on-surface-variant leading-relaxed">
                                        DL: {new Date(result.sla_deadline).toLocaleString()}
                                    </p>
</div>

{result.priority_reason && (
<div className="bg-tertiary-fixed/10 p-3 rounded-md border border-tertiary-fixed/20">
    <h4 className="font-label text-[10px] uppercase tracking-wider text-tertiary-fixed-dim mb-1 flex items-center gap-1">
        <span className="material-symbols-outlined text-[12px]">analytics</span>
        ML Priority Explanation
    </h4>
    <p className="font-body text-xs text-on-surface leading-relaxed mb-2">
        {result.priority_reason}
    </p>
    
    <div className="flex flex-wrap gap-1">
        {result.top_keywords?.category_keywords?.map(kw => (
            <span key={`cat-${kw}`} className="px-1.5 py-0.5 bg-primary/10 text-primary text-[9px] uppercase tracking-wider font-bold rounded">
                Cat: {kw}
            </span>
        ))}
        {result.top_keywords?.priority_keywords?.map(kw => (
            <span key={`pri-${kw}`} className="px-1.5 py-0.5 bg-error-container text-on-error-container text-[9px] uppercase tracking-wider font-bold rounded">
                Pri: {kw}
            </span>
        ))}
    </div>
    
    {result.class_probabilities?.category && Object.keys(result.class_probabilities.category).length > 0 && (
        <div className="mt-2 pt-2 border-t border-tertiary-fixed/20">
            <div className="text-[9px] uppercase text-secondary mb-1 font-bold">Class Probabilities</div>
            <div className="flex gap-2">
                {Object.entries(result.class_probabilities.category).map(([cls, prob]) => (
                    <div key={cls} className="flex-1">
                        <div className="flex justify-between text-[8px] mb-0.5 font-medium">
                            <span className="truncate">{cls}</span>
                            <span>{(prob*100).toFixed(0)}%</span>
                        </div>
                        <div className="w-full bg-surface-container-highest h-1 rounded-full overflow-hidden">
                            <div className="bg-primary h-full" style={{width: `${prob*100}%`}}></div>
                        </div>
                    </div>
                ))}
            </div>
